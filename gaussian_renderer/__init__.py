...
from scene import GaussianModel, UniGaussians
...
def render(viewpoint_camera, pc : Union[GaussianModel, UniGaussians], pipe, \
    bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, iter = 0, specular_color = None
        ):
    """
    Render the scene. 
    ...
    if pipe.DTF: #! to consensus with the convention with glm, matrices must be transposed as input. 
        ...
        if pipe.train_kinematic:
            ...
            if pipe.invT_Jacobian:
                ...
                Jacobians[-pc.hand_gaussian_model.binding.shape[0]:] = pc.hand_gaussian_model.face_trans_mat[pc.hand_gaussian_model.binding].permute(0,2,1)
                Jacobians_inv[-pc.hand_gaussian_model.binding.shape[0]:] = torch.linalg.inv(pc.hand_gaussian_model.face_trans_mat)[pc.hand_gaussian_model.binding]
...
def render_contact(viewpoint_camera, pc : Union[GaussianModel, UniGaussians], pipe, \
    bg_color : torch.Tensor, scaling_modifier = 1.0, contact_color = None
        ):
    """
    Render the scene. 
    ...
    # asset_bucket = {}
    # Create zero tensor. We will use it to make pytorch return gradients of the 2D (screen-space) means
    screenspace_points = torch.zeros_like(pc.hand_gaussian_model.get_xyz, dtype=pc.hand_gaussian_model.get_xyz.dtype, requires_grad=True, device="cuda") + 0
    try:
        screenspace_points.retain_grad()
    except:
        pass
    ...
    means3D = means3D[-pc.hand_gaussian_model.binding.shape[0]:]
    means2D = screenspace_points
    
    opacity = pc.get_opacity[-pc.hand_gaussian_model.binding.shape[0]:]
    ...
    if pipe.convert_SHs_python:
        shs_view = pc.get_features.transpose(1, 2).view(-1, 3, (pc.max_sh_degree+1)**2)
        dir_pp = (pc.get_xyz - viewpoint_camera.camera_center.repeat(pc.get_features.shape[0], 1).cuda())
        dir_pp_normalized = dir_pp/dir_pp.norm(dim=1, keepdim=True)
        if pipe.rotSH:
            if pipe.train_kinematic:
                dir_pp_normalized = (pc.blended_R.permute(0,2,1) @ dir_pp_normalized[...,None]).squeeze(-1)
    ...
    if contact_color is not None:
        # colors_precomp[-pc.hand_gaussian_model.binding.shape[0]:] = contact_color
        colors_precomp = contact_color
    scales = scales[-pc.hand_gaussian_model.binding.shape[0]:]
    rotations = rotations[-pc.hand_gaussian_model.binding.shape[0]:]
    rendered_image, radii, allmap, n_contrib_pixel, top_weights, top_depths, visible_points_tight = rasterizer(                                                                                   
