...
import dataclasses
import trimesh
from utils.brics_utils.structures import *
...
def concat_img_array(im1, img2, dim = 0):
    if is_tensor(im1):
        img = torch.cat((im1, img2), dim=dim)
    elif is_numpy(im1):
        img = np.concatenate((im1, img2), axis=dim).astype(np.uint8)
    return img
...
def dump_points(path, points, colors=None):
