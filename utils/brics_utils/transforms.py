...
def vector_norm(vecA: torch.Tensor, dim: int = -1) -> torch.Tensor: 
    return torch.linalg.vector_norm(vecA, dim = dim)

def matrix_norm(A: torch.Tensor) -> torch.Tensor: 
    return torch.linalg.matrix_norm(A)

def normalize(vecA: torch.Tensor, dim : int) -> torch.Tensor:
    return vecA / vector_norm(vecA)

def einsum(expr: str, A: torch.Tensor, B: torch.Tensor) -> torch.Tensor: 
    return torch.einsum(expr, A, B)

def cross(vecA: torch.Tensor, vecB: torch.Tensor) -> torch.Tensor: 
    return torch.cross(vecA, vecB, dim = -1)

def hadmard(A: torch.Tensor, B: torch.Tensor) ->  torch.Tensor: 
    return torch.mul(A, B)

def sample_points_inside_triangle(triangles, num_points): 
    ...
    a = np.random.uniform(0, 1, num_points)
    b = np.random.uniform(0, 1, num_points)
    t1 = 1 - np.sqrt(a)
    t2 = np.sqrt(a) * (1 - b)
    t3 = b * np.sqrt(a)
    
    sampled_points = np.zeros((triangles.shape[0], num_points, 3))
    for i in range(triangles.shape[0]): 
        t = t1[..., None] * np.tile(triangles[i][0], (num_points, 1)) + \
            t2[..., None] * np.tile(triangles[i][1], (num_points, 1)) + \
            t3[..., None] * np.tile(triangles[i][2], (num_points, 1)) 
        sampled_points[i] = t

    return sampled_points

def get_barycentric_coordinates(query_vertices: torch.Tensor, triangle_vertices: torch.Tensor) -> torch.Tensor: 
    assert(query_vertices.shape[0] == triangle_vertices.shape[0])

    x0 = triangle_vertices[..., 0, :]
    x1 = triangle_vertices[..., 1, :]
    x2 = triangle_vertices[..., 2, :]

    x0x1 = x0 - x1
    x0x2 = x0 - x2

    triangle_area = vector_norm(cross(x0x1, x0x2)) / 2

    ## if query vertices are named `q`
    qx0 = query_vertices - x0
    qx1 = query_vertices - x1
    qx2 = query_vertices - x2

    area_qx0x1 = vector_norm(cross(qx0, qx1)) / 2
    area_qx0x2 = vector_norm(cross(qx0, qx2)) / 2
    area_qx1x2 = vector_norm(cross(qx1, qx2)) / 2
    
    ## If barycentric coordinates are given by u, v, w corresponding to x0, x1 and x2
    u = area_qx1x2 / triangle_area
    v = area_qx0x2 / triangle_area
    w = 1 - u - v

    ## Sanity check new_vertices should match query_vertices
    # new_vertices = u[..., None] * x0 + v[..., None] * x1 + w[..., None] * x2
    return torch.stack([u, v, w], dim = -1)

def interpolate_vertex_data_using_barycentric_coordinates(b_weights: torch.Tensor, per_face_data: torch.Tensor) -> torch.Tensor: 
    u, v, w = b_weights[..., 0], b_weights[..., 1], b_weights[..., 2]
    d0 = per_face_data[..., 0, :]
    d1 = per_face_data[..., 1, :]
    d2 = per_face_data[..., 2, :]

    interpolated_weights = u[..., None] * d0 + v[..., None] * d1 + w[..., None] * d2
